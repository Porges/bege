...
type Optimizer (itoh: Xunit.Abstractions.ITestOutputHelper) = 
    ...
    let ``eventualFate: OutputNumber is a consume`` () =
        Assert.Equal(Consumed, eventualFate { control = ToState 0; instructions = [OutputNumber] })
        
    [<Fact>]
    let ``eventualFate: Discard is detected after several instructions`` () =
        Assert.Equal(Discarded, eventualFate { control = ToState 0; instructions = [InputNumber; Discard; Discard] })

    [<Fact>]
    let ``eventualFate: Consume is detected after several instructions`` () =
        Assert.Equal(Consumed, eventualFate { control = ToState 0; instructions = [InputNumber; Discard; OutputNumber] })

        
    [<Fact>]
    let ``eventualFate: slightly more complex`` () =
        Assert.Equal(Discarded, eventualFate { control = ToState 0; instructions = [InputNumber; InputNumber; BinOp Add; OutputNumber; Discard] })
